...
export const numberRowRule = rule('Number Row Symbols').manipulators([
  ...
  map(2, '⇧')
    ...
    .condition(notEn()),
  map(3, '⇧').to(toKey(3, '⇧')).condition(en()),
  map(3, '⇧')
    .to(system.ABCLayout)
    .toAfterKeyUp([toKey(3, '⇧'), system.russianLayout])
    .condition(notEn()),
  map(4, '⇧').to(toKey(6, '⇧')).condition(en()),
  map(4, '⇧')
    .to(system.ABCLayout)
  ...
  map(6, '⇧')
    .to(system.ABCLayout)
    .toAfterKeyUp([toKey("'"), system.russianLayout])
    .condition(notEn()),
  map(8, '⇧').to(toKey(8, '⇧')),
  map(9, '⇧').to(toKey(',', '⇧')).condition(en()),
  map(9, '⇧')
    .to(system.ABCLayout)
    .toAfterKeyUp([toKey(',', '⇧'), system.russianLayout])
    .condition(notEn()),
  map(0, '⇧').to(toKey('.', '⇧')).condition(en()),
  map(0, '⇧')
    .to(system.ABCLayout)
    .toAfterKeyUp([toKey('.', '⇧'), system.russianLayout])
    .condition(notEn()),
